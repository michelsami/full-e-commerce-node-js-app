    ...
    if (err) {
      ...
      if (err.name === 'TokenExpiredError') {
        ...
        jwt.verify(refreshToken, process.env.JWT_SECRET_REFRESH, async(err, decoded) => {
          ...
          const accessToken =  jwt.sign({ email: decoded.email, name :decoded.name}, process.env.JWT_SECRET, { expiresIn: '15m' });
...
export const generateAcessAndRefreshTokens = (pyload)=>{
...
export const createUser = asyncHandler(async(req, res)=>{
	...
	const newUser = {name: name, password: password, email: email, slug:slugify(name), refreshToken: tokens.refreshToken, accessToken:tokens.accessToken}
